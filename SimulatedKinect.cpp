...
//-----------------------------------------------------------------------------------//
// Global variable for interaction on the robotic.
//-----------------------------------------------------------------------------------//
double rx = 0;
double ry = 0;
double rz = 0;
...
void KeypressCallbackFunction(vtkObject* caller, long unsigned int vtkNotUsed(eventId), void* clientData, void* vtkNotUsed(callData) )
{
   ...
   vtkActorCollection* actorCollection = static_cast<vtkActorCollection*>(clientData);
   ...
   if(key == "Up")
   {
		 ...
		 std::cout << "Position: (" << rx << " , " << ry << " , " << rz << "). " << std::endl;
   }
   ...
   vtkSmartPointer<vtkTransform> robotTransform = vtkSmartPointer<vtkTransform>::New();
   robotTransform->RotateX(90);
   robotTransform->Scale(0.25, 0.25, 0.25);
   robotTransform->Update();

   actorCollection->InitTraversal();

   vtkSmartPointer<vtkRenderWindow> renderWindow = rwi->GetRenderWindow();
   vtkSmartPointer<vtkRendererCollection> renderCollection = renderWindow->GetRenderers();
   vtkSmartPointer<vtkRenderer> renderer = renderCollection->GetFirstRenderer();

   for(vtkIdType i = 0; i < actorCollection->GetNumberOfItems(); i++)
   {
		vtkSmartPointer<vtkActor> nextActor = actorCollection->GetNextActor();
		//nextActor->SetTexture(texture);
		//nextActor->SetPosition(rx, ry, rz);
		nextActor->SetPosition(rx, rz, ry);
		nextActor->SetUserTransform(robotTransform);

		nextActor->VisibilityOn();

		renderer->AddActor(nextActor);

		//this->GetCurrentRenderer()->AddActor(nextActor);
		//this->InteractionProp = nextActor;

   }

   // Enter the "Enter/Return" key to take a RGBD picture.
   if(key == "Return")
   {
		...
		writer->SetFileName("capture_rgb.png");
		...
		imageWriter->SetFileName("capture_depth.bmp");
...
#include <vtkPNGReader.h>
	...
	// Add a robot into the world.
	string robot_meshfile = "../robot/dvfpebi281kw-Cyborg/Cyborg.obj";
	string robot_meshMTLfile = "../robot/dvfpebi281kw-Cyborg/Cyborg.mtl";
	vtkSmartPointer<vtkOBJImporter> Robot_importer = vtkSmartPointer<vtkOBJImporter>::New();
	Robot_importer->SetFileName(robot_meshfile.data());
	Robot_importer->SetFileNameMTL(robot_meshMTLfile.data());

	//--------------------------------------------------------------------------------//
	// Visualization
	vtkSmartPointer<vtkRenderer> robot_render = vtkSmartPointer<vtkRenderer>::New();
	vtkSmartPointer<vtkRenderWindow> robot_renderWindow = vtkSmartPointer<vtkRenderWindow>::New();

	robot_renderWindow->AddRenderer(robot_render);
	
	Robot_importer->SetRenderWindow(robot_renderWindow);
	Robot_importer->Update();
	
	
	vtkSmartPointer<vtkTransform> robotTransform = vtkSmartPointer<vtkTransform>::New();
	robotTransform->RotateX(90);
	robotTransform->Scale(0.25, 0.25, 0.25);
	robotTransform->PostMultiply();
	robotTransform->Update();

	//robotTransform->Concatenate(gTransform->GetMatrix());
	//robotTransform->Update();

	vtkActorCollection* robotActorCollection = robot_render->GetActors();
	//vtkSmartPointer<vtkActorCollection> robotActorCollection = robot_render->GetActors();
	robotActorCollection->InitTraversal();
	
	for(vtkIdType i = 0; i < robotActorCollection->GetNumberOfItems(); i++)
	{
		vtkSmartPointer<vtkActor> nextActor = robotActorCollection->GetNextActor();
		//nextActor->SetTexture(texture);
		nextActor->SetUserTransform(robotTransform);
		double *bounds = nextActor->GetBounds();	
		boundingbox.AddBounds(bounds);
	...
	*/
	...
	keypressCallback->SetClientData(robotActorCollection);
