...
def vertex_format_indices(vertex_format)
  puts "vertex_format : " + vertex_format.to_s  
  p = 0
...
def read_triangles_inputs(triangles_elem)
  ...
  triangles_elem.elements.each('input') do |input_elem|
	puts "input_element:  " + input_elem.to_s + "\n"  
...
def read_triangles(triangles_elem, id_elem_hash, material_symbol_hash, material_count_hash)
  puts "triangles_elem:  " + triangles_elem.to_s + "\n"
  inputs = read_triangles_inputs(triangles_elem)
  puts "inputs[:vertex]: \n"
  
  vertex_elem = id_elem_hash[inputs[:vertex][:source]]
  vertex_elem.elements.each('input') do |input_elem|
	puts "input_element:  " + input_elem.to_s + "\n"  

    semantic = get_attr_str(input_elem, 'semantic')
    source = read_url(input_elem, 'source')
    offset = 0 #get_attr_int(input_elem, 'offset')
    set_str = input_elem.attributes['set']
    set = set_str == nil ? 0 : str_to_int(set_str)

    hash_val = {:source => source, :offset => offset}
	if semantic.upcase == "POSITION" and inputs[:position] == nil
        inputs[:position] = hash_val
	end

	if semantic.upcase == "NORMAL" and inputs[:normal] == nil
		inputs[:normal] = hash_val
	end
  ...
  puts "Position ,  " + inputs[:position][:source].to_s + "\n"
  puts "Noraml,  " + inputs[:normal][:source] + "\n"

  positions = read_source(inputs[:position][:source], id_elem_hash, 'X', 'Y', 'Z')
  normals =  read_source(inputs[:normal][:source], id_elem_hash, 'X', 'Y', 'Z') 
  texcoords = inputs[:texcoord] ? read_source(inputs[:texcoord][:source], id_elem_hash, 'S', 'T') : nil

  puts "vertexs: " + positions.to_s + "\n"
  puts "normals: " + normals.to_s + "\n"
  puts "texcoords: " + texcoords.to_s + "\n"
  ...
  puts "\n" + "material_symbol_hash:  "  + material_symbol_hash.to_s + "\n"
  puts "\n" + "material_count_hash:  "  + material_count_hash.to_s + "\n"
...
class Material
  ...
  attr_accessor :name, :ambient, :diffuse, :specular, :emission, :d, :ns, :illumination 

  def initialize(name, ambient=[0,0,0], diffuse=[0,0,0], specular=[0,0,0], emission=[0,0,0], d=1, ns=10, illumination=2)
    @name = name
    @ambient = ambient
    @diffuse = diffuse
	@specular = specular
  ...
  def ==(material)
    if @name != material.name or
        ...
        diffuse_type != material.diffuse_type or
        specular_type != material.specular_type or   
        emission_type != material.emission_type 
      return false
    end
    ...
    if specular_type == :texture  and  @specular != material.specular
      return false
    elsif specular_type == :color  and  !nested_array_equal(@specular, material.specular)
      return false
    end

    if emission_type == :texture  and  @emission != material.emission
      return false
    elsif emission_type == :color  and  !nested_array_equal(@emission, material.emission)
      return false
    end
  ...
  def initialize_copy(source)
    ...
    @emission = source.emission.clone
    @d = source.d
    @ns = source.ns
  ...
  def specular_type()
    Material.material_property_type(@specular)
  end

  def emission_type()
    Material.material_property_type(@emission)
  end
...
def read_material(material_elem, id_elem_hash, material_count_hash)
  ...
  material.specular = read_material_property(lambert_elem, 'specular', effect_elem, id_elem_hash)
  material.emission = read_material_property(lambert_elem, 'emission', effect_elem, id_elem_hash)
  material
end
...
def traverse_node(node_elem, parent_transform, id_elem_hash, material_count_hash)
  ...
  node.instance_geoms.each do |instance_geom|
	...
	puts "geom_elem:  " + geom_elem.to_s + "\n"
	puts "geom_meshes:  " + geom_meshes.to_s + "\n"
...
def to_obj(filename, meshes, material_file_name)
  print_vertex = lambda do |io, vertex, vertex_format|
	...
	#puts "p_index = " + p_index.to_s + ", n_index = " + n_index.to_s + ", t_index = " + t_index.to_s 
...
def to_mtl(filename, materials, specific_dir)
  ...
  materials.each do |material|
    ...
    print_material_property.call(file, material.specular, 'Ks')
    print_material_property.call(file, material.emission, 'Ke')
	file.printf("d 1\n")
	file.printf("Ns 10\n")
	file.printf("illum 2\n")
    file.printf("\n")
...
class DiskCopier
  ...
  def copy(from_file, to_file)
    #full_from_file = File.expand_path(from_file) #File.expand_path to convert the relative path to the absolute path.
    full_from_file = from_file #File.expand_path to convert the relative path to the absolute path.
...
def fix_material_image_references(materials)
  ...
  materials.each do |material|
    ...
    image_refs.each do |image_ref|
	  puts "image_ref_suffix: " + path_basename_and_suffix(image_ref) + "\n"
...
if $0 == __FILE__
  ...
  #specific_dir = "/../worlds/models/" + output_basename + "/" #only for my current setting to visulize the *.obj and *.obj.mtl. 
  specific_dir = ""  
  if ARGV.count > 1
    output_dir = ARGV[1]
  end
  ...
  if options[:blender_shrink]
    ...
    transform = matrix_mult(x_rotation_matrix(-Math::PI/2), uniform_scale_matrix(0.025))
    meshes = meshes.map { |mesh| pretransform_mesh(mesh, transform) }
  end

  obj_file_name = path_join(output_dir, output_basename + '.obj')
  mtl_file_name = path_join(output_dir, output_basename + '.obj.mtl')
  ...
  to_mtl(mtl_file_name, materials, specific_dir)
